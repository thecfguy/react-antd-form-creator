...
const FormViewer = ({ elements, values, descriptionProps, descriptionItemProps, ...props }) => {
    const displayObject = elements.reduce((acc, element, index) => {
        if (element.type === "Header" || element.type === "Paragraph" || element.type === "LineBreak" || element.type === "Image")
            acc.push(element);
        else {
    ...
    const displayElement = () => {
        return displayObject.map(element => {
            ...
            if (Array.isArray(element)) {
                return (<Descriptions {...descriptionProps}>
                    {
                        element.map((e, index) => {
                            const value = values?.[e.field_name];
                            return <Descriptions.Item key={e.id} label={e.label} {...descriptionItemProps}><ViewElement key={e.id} element={e} value={value} /></Descriptions.Item>;
                        })
                    }
                </Descriptions>)
            }
